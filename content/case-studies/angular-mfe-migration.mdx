---
title: "Angular Migration Across Shared Microfrontends"
...
subtitle: "A framework jump, an ag-Grid major version upgrade, and a rollout designed around teams we didn't control."
status: "Shipped"
year: 2026
...
The application runs in production with regular release cycles, and the rollout plan had to support an instant switch between the Angular 13 baseline and Angular 18 at any point — including after go-live if something went wrong.
...
Keeping both Angular 13 and Angular 18 clusters running simultaneously meant running double the ECS capacity for the duration of the rollout period. That was an accepted cost — having a rollback path that required no code change or pipeline trigger was worth the extra capacity.
...
Because sign-off was gated on each downstream team's QA availability, the critical path was determined by scheduling rather than technical complexity. Flagging this dependency early — and keeping each team informed of the timeline — was the main thing that prevented a stall at the end of the cycle.
...
A few things from this migration are now defaults:

- Configuration-driven host routing as a rollout mechanism — no deployment pipeline required.
- Shared wiki documentation when multiple developers hit the same breaking change, rather than each one solving it independently.
- Letting visible load failures drive upgrade order rather than pre-planned sequencing.
- Identifying cross-team QA dependencies at the start of planning, not at sign-off.
